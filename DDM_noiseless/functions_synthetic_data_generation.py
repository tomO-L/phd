...
def run_simulation(args_dict):
    ...
    for _ in range(steps_number):
        ...
        drift = np.sum(np.array([action,1-action]) * drift)

        """
        BON C'EST FAUX MAIS TQT
                                                                 Rewarded or not
                    |-----------------------------------------------------|----------------------------------------------------|
                    | action = 1 (CW) and reward = 1 --> + reward_drift   | action = 1 (CW) and reward = 0 --> - reward_drift  |
        Action type |-----------------------------------------------------|----------------------------------------------------|
                    | action = 0 (CCW) and reward = 1 --> - failure_drift | action = 0 (CCW) and reward = 0 --> + failure_drift|
                    |-----------------------------------------------------|----------------------------------------------------|
        """
...
def run_simulations_batch_random_init(args_dict, n_simulations, p_cw_init_range):
