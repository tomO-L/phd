...
def fit_hmm_fixed_states_number_v2(x_train, x_validate, training_lengths, validation_lengths, n_states, n_fits=200, n_features=None, fix_probability=False):
    ...
    for idx in range(n_fits):
        ...
        if not(fix_probability):
            ...
            model.emissionprob_ = np.transpose(np.array([1-np.linspace(0,1,n_states+2)[1:-1],np.linspace(0,1,n_states+2)[1:-1]]))
...
def infer_best_model_score_parallel_v2(x_train, x_validate, training_lengths, validation_lengths, n_to_test, fix_probability=False, n_features = None, n_fits=200, n_jobs=-2):
    # check optimal score

    models_and_scores_list = jl.Parallel(n_jobs=n_jobs)(jl.delayed(fit_hmm_fixed_states_number_v2)(x_train, x_validate, training_lengths, validation_lengths, n_states, 
                                                                                                fix_probability=fix_probability, n_fits=n_fits, n_features=n_features)
                                                                                                for n_states in n_to_test)
...
def infer_best_model_score_v2(x_train, x_validate, training_lengths, validation_lengths, n_to_test, fix_probability=False, n_features = None, n_fits=200, n_jobs=-2):
    # check optimal score
    models_and_scores_list = []

    for n_states in tqdm(n_to_test):

        res = fit_hmm_fixed_states_number_v2(x_train, x_validate, training_lengths, validation_lengths, n_states, 
                                    fix_probability=fix_probability, n_fits=n_fits, n_features=n_features)
