...
for mouse in mice_to_analyse:
    ...
    for session_index, session_to_process in enumerate(sessions_to_process):
        ...
        if not os.path.exists(output_pickle_filepath):
            ...
            int_nw_time_ratio_persession[mouse].append([session_index + 1, np.nan])
            int_ne_time_ratio_persession[mouse].append([session_index + 1, np.nan])
            int_se_time_ratio_persession[mouse].append([session_index + 1, np.nan])
            int_sw_time_ratio_persession[mouse].append([session_index + 1, np.nan])
