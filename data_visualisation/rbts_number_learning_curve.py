...
int_start_trap_ratio_persession = {mouse: [] for mouse in mice_to_analyse}
ext_start_trap_ratio_persession = {mouse: [] for mouse in mice_to_analyse}
int_end_trap_ratio_persession = {mouse: [] for mouse in mice_to_analyse}
ext_end_trap_ratio_persession = {mouse: [] for mouse in mice_to_analyse}
...
for mouse in mice_to_analyse:
    ...
    for session_index, session_to_process in enumerate(sessions_to_process):
        ...
        if not os.path.exists(output_pickle_filepath):
            ...
            int_start_trap_ratio_persession[mouse].append([session_index + 1, np.nan])
            ext_start_trap_ratio_persession[mouse].append([session_index + 1, np.nan])
            int_end_trap_ratio_persession[mouse].append([session_index + 1, np.nan])
            ext_end_trap_ratio_persession[mouse].append([session_index + 1, np.nan])
        ...
        ratio_int_start_trap = compute_int_start_trap(rbts)/n_rbts
        ratio_ext_start_trap = compute_ext_start_trap(rbts)/n_rbts
        ratio_int_end_trap = compute_int_end_trap(rbts)/n_rbts
        ratio_ext_end_trap = compute_ext_end_trap(rbts)/n_rbts

        int_start_trap_ratio_persession[mouse].append([session_index + 1, ratio_int_start_trap])
        ext_start_trap_ratio_persession[mouse].append([session_index + 1, ratio_ext_start_trap])
        int_end_trap_ratio_persession[mouse].append([session_index + 1, ratio_int_end_trap])
        ext_end_trap_ratio_persession[mouse].append([session_index + 1, ratio_ext_end_trap])
...
plot_learning_curve(int_start_trap_ratio_persession, mice_to_analyse, ax1, wilcoxon_test_h0=0.5)
ax1.set_ylabel('Ratio of\n starting interior trapeze')
ax1.set_ylim([0,1])
ax1.axhline(0.5,0,20, color='grey', linestyle='--')

plot_learning_curve(int_end_trap_ratio_persession, mice_to_analyse, ax2, wilcoxon_test_h0=0.5)
ax2.set_ylabel('Ratio of\n ending interior trapeze')
ax2.set_ylim([0,1])
ax2.axhline(0.5,0,20, color='grey', linestyle='--')

plot_learning_curve(ext_start_trap_ratio_persession, mice_to_analyse, ax3, wilcoxon_test_h0=0.5)
ax3.set_ylabel('Ratio of\n starting exterior trapeze')
ax3.set_ylim([0,1])
ax3.axhline(0.5,0,20, color='grey', linestyle='--')

plot_learning_curve(ext_end_trap_ratio_persession, mice_to_analyse, ax4, wilcoxon_test_h0=0.5)
ax4.set_ylabel('Ratio of\n ending exterior trapeze')
ax4.set_ylim([0,1])
