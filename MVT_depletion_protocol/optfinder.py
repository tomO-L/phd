...
rate_threshold_list = np.linspace(0.01,2,1000)
...
patch_types = [[harvest_function_1, exploitation_effort_function]]


max_average_intake_rate, opt_leaving_times = opt_finder_v2(t_travel, rate_threshold_list, patch_types, travel_effort_function)
...
print("opt_leaving_times = ", opt_leaving_times)
...
if display_plots:
    ...
    for i, patch_type in enumerate(patch_types):
        ...
        t_max = opt_leaving_times[i]
        chronology = np.linspace(0,t_max, int(t_max/dt))
        ...
        print(compute_instant_intake_rate(patch_type[i][0], exploitation_effort_function, opt_leaving_times, dt))

        ax.axvline(opt_leaving_times[i], linestyle='--', color='grey', label = f'Patch {i} opt. leaving time')
