...
with open(f'DDM/statistical_precision_analysis/simulations_batches/simulations_batch_{example_n_simulations}_test.pkl', 'rb') as file:
    synthetic_data = dill.load(file)

test_data = [synth_data['choices'] for synth_data in synthetic_data]

with open(f'DDM/statistical_precision_analysis/simulations_batches/best_model_score_{example_n_simulations}_fulltraining_2.pkl', 'rb') as file:
...
for delta in tqdm(delta_range):
    ...
    mean_probability = compute_simulations_average(p_a, p_a_reward, steps_number, noise_amplitude, delta, drift, n_simulations=5000)
...
ax1.set_ylabel('Probability to chose 1')
...
for n_simulations in n_simulations_list:
    ...
    with open(f'DDM/statistical_precision_analysis/simulations_batches/mse_{n_simulations}_fulltraining_2.pkl', 'rb') as file: ### NEED MSE
